...
export function PermuterContribution({ data, runLabels }: Props) {
  ...
  const runStats = runLabels.map((_, ri) => {
    ...
    for (const func of data.functions) {
      ...
      // Count programmatic-phase permuter matches (solved before AI attempts)
      if (run.matchSource === 'decomp-permuter' && run.attempts.length === 0) {
        solved++;
      }
    ...
    <div>
        ...
        {runLabels.map((label, i) => (
          <div key={i} className="bg-slate-800/60 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div>
                <div className="text-slate-500 text-xs">Solved (total)</div>
      ...
      <div className="bg-slate-800/40 rounded-lg p-4 border border-slate-700">
        ...
        <ul className="text-slate-400 text-sm space-y-1">
          <li>
            The chart above shows background permuter tasks that ran concurrently with Claude attempts.
            These scores reflect improvement attempts during the retry loop.
          </li>
          <li>
            Additionally, func_80954E0C_jp was solved by the permuter during the programmatic phase
            (before AI attempts, 0 retries needed) in all 3 runs. The other permuter matches
            (func_809D18C8_jp, func_808BB604_jp, func_808D6BAC_jp) were solved by the background
            permuter during the AI retry loop.
          </li>
          <li>
            Score improvements in the chart indicate the permuter found better register allocation or instruction
            ordering, but never fully solved additional functions beyond the programmatic phase matches.
          </li>
        </ul>
      </div>
    </div>
