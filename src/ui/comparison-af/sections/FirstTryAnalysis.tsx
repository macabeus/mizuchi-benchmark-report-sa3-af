...
export function FirstTryAnalysis({ data, runLabels }: { data: ComparisonData; runLabels: string[] }) {
  ...
  // Functions that matched on first try in ALL runs (excluding programmatic-phase)
  ...
  return (
    <div>
      ...
      {/* Note about programmatic matches */}
      <div className="bg-slate-800/30 rounded-lg p-3 mb-6 border border-slate-700/30">
        <p className="text-slate-400 text-xs">
          <span className="text-slate-300 font-medium">Note:</span> This analysis excludes functions matched during the
          programmatic phase (m2c/permuter with 0 AI attempts). AF has 5 such functions per run that are excluded from
          the denominator. The rates above reflect only functions that required Claude&apos;s AI intervention.
        </p>
      </div>
      ...
      <div className="bg-slate-800/40 rounded-lg p-4">
        ...
        <p className="text-slate-500 text-xs mb-3">
          ...
          PROG = matched during programmatic phase (m2c/permuter, 0 AI attempts).
        </p>
        <table className="w-full text-xs">
          ...
          <tbody>
            {data.functions
              .filter((f) => f.runs.some((r) => r.numAttempts > 0 || r.matchSource === 'programmatic-phase'))
              .map((func) => (
