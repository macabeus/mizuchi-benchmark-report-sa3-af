...
export function TimeoutSummary({ data, runLabels }: Props) {
  ...
  // TTFT timeout was only implemented starting Run 3
  const hasTtft = data.runs.map((r) => r.config.ttftTimeoutMs !== null);
  ...
  return (
    <div>
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div>
                ...
                <div className="text-white font-medium">
                  {hasTtft[i] ? runTotals[i].ttft : <span className="text-slate-600" title="TTFT timeout not implemented in this run">&ndash;</span>}
                </div>
              </div>
              <div>
      ...
      {/* TTFT note */}
      <div className="bg-purple-900/20 border border-purple-500/30 rounded-lg px-4 py-3 mb-6 text-sm text-purple-300">
        <span className="font-medium">Note:</span> TTFT (time-to-first-token) timeout was only implemented starting
        with Run 3. Runs 1 and 2 did not have this timeout type, so their TTFT counts are shown
        as &ldquo;&ndash;&rdquo;. Before TTFT timeout existed, slow API responses that never started generating
        would eventually hit the hard timeout instead.
      </div>
                ...
                <tr key={func.functionName} className="border-b border-slate-800">
                  ...
                  {func.runs.map((run, ri) => (
                    <td key={ri} className="py-2 px-2 text-center text-xs">
                      ...
                      {hasTtft[ri] && run.ttftTimeouts > 0 && (
                        <span className="inline-block px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-400 mr-1">
                          T:{run.ttftTimeouts}
                        </span>
                      )}
                      {run.softTimeouts === 0 && run.hardTimeouts === 0 && (!hasTtft[ri] || run.ttftTimeouts === 0) && (
